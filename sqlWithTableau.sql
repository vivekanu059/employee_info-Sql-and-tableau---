...
SELECT 
    e.gender, 
    d.dept_name, 
    ROUND(AVG(CAST(s.salary AS FLOAT)), 2) AS salary, 
    YEAR(s.from_date) AS calendar_year
FROM t_employees e
JOIN t_dept_emp de ON de.emp_no = e.emp_no
JOIN t_departments d ON d.dept_no = de.dept_no
JOIN t_salaries s ON s.emp_no = e.emp_no
GROUP BY d.dept_no, d.dept_name, e.gender, YEAR(s.from_date)
HAVING YEAR(s.from_date) <= 2002
ORDER BY d.dept_no;
...
CREATE PROCEDURE getAverageSalrayMaleVsFemale (
    @p_min_salary FLOAT, 
    @p_max_salary FLOAT
)
AS
BEGIN
	...
	SELECT 
        e.gender, 
        d.dept_name, 
        ROUND(AVG(CAST(s.salary AS FLOAT)), 2) AS avg_salary
	FROM t_employees e
	JOIN t_dept_emp de ON de.emp_no = e.emp_no
	JOIN t_departments d ON d.dept_no = de.dept_no
	JOIN t_salaries s ON s.emp_no = e.emp_no
...
EXEC getAverageSalrayMaleVsFemale @p_min_salary = 50000, @p_max_salary = 90000;
GO
